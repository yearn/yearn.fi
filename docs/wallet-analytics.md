...
Both apps default to the existing `yearn.fi` site, use `/proxy/plausible/api/event`, and can override the site with `NEXT_PUBLIC_PLAUSIBLE_DOMAIN`. Both apps disable localhost collection by default; `NEXT_PUBLIC_PLAUSIBLE_TRACK_LOCALHOST=true` enables explicit local QA. Intercept event requests during automated QA to keep test traffic out of production analytics. No dashboard configuration is performed by this code change.

For a preview registered as a separate site in Plausible, set `NEXT_PUBLIC_PLAUSIBLE_DOMAIN` to that exact site identifier (hostname only, without `https://` or a path) in the Vercel Preview environment for the relevant branch. For PR #1398's registered preview, use `yearnfi-git-codex-rainbow-wallet-ui-plausbile-yearn.vercel.app` on branch `codex/rainbow-wallet-ui-plausbile`. Leave Production unset or set to `yearn.fi`. Rebuild/redeploy after changing it: Next.js embeds public environment variables at build time. Then rerun installation verification in the matching Plausible site. Merely registering a preview site in Plausible does not change where the application sends events.
