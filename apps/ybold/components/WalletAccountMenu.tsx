        ...
        <div className="mb-7 flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            >
              ...
              {wasCopied && (
                <span role="status" className="ml-1 text-xs font-normal text-good">
                  Copied
                </span>
              )}
            </button>
...
export function WalletAccountMenu() {
  ...
  const boldPrice = getUsdPrice(YBOLD_PRICE_TOKENS[0])
  const positionValue =
    Number(formatUnits(vaultUserData.depositedValue, vaultUserData.assetToken?.decimals ?? 18)) * boldPrice
  ...
  const showView = (nextView: TAccountView) => {
    setView(nextView)
    requestAnimationFrame(() =>
      document.getElementById(nextView === 'settings' ? 'ybold-wallet-settings-back' : 'ybold-wallet-settings')?.focus()
    )
  }
  ...
  const handleDisconnect = useCallback(async () => {
    ...
    try {
      ...
      setDisconnectError('Unable to disconnect. Please try again.')
    }
  ...
  return (
    <div className="relative">
      ...
      <WalletSurface
        isOpen={isOpen}
        onClose={closeMenu}
        title={view === 'settings' ? 'Wallet settings' : 'Wallet account'}
        dialogId="ybold-wallet-account-menu"
        desktopTop="5rem"
      ...
      >
        ...
        {view === 'account' ? (
          <AccountView
            address={address}
            isBalanceLoading={vaultUserData.isLoading || (isSpotPriceLoading && boldPrice === 0)}
