...
export const REDIS_MISSING_CONFIGURATION_MESSAGE =
  'UPSTASH_REDIS_REST_URL_DOA and UPSTASH_REDIS_REST_TOKEN_DOA must both be set'
...
function getClient(): Redis {
  if (!client) {
    const url = process.env.UPSTASH_REDIS_REST_URL_DOA
    const token = process.env.UPSTASH_REDIS_REST_TOKEN_DOA
    if (!url || !token) {
