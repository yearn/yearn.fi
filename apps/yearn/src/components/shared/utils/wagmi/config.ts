...
export function getConfig({ chains }: { chains: Chain[] }): ResolvedRegister['config'] {
  ...
  for (const chain of chains) {
    ...
    const oldRPC = env.NEXT_PUBLIC_JSON_RPC_URI?.[chain.id] || env.NEXT_PUBLIC_JSON_RPC_URL?.[chain.id]
     ...
     *************************************************************************************/
    if (getNetwork(chain.id)?.rpcUrls.alchemy?.http[0] && env.NEXT_PUBLIC_ALCHEMY_KEY) {
      availableTransports.push(
        http(`${getNetwork(chain.id)?.rpcUrls.alchemy.http[0]}/${env.NEXT_PUBLIC_ALCHEMY_KEY}`, {
          batch: true
        })
      )
    }
    if (getNetwork(chain.id)?.rpcUrls.infura?.http[0] && env.NEXT_PUBLIC_INFURA_PROJECT_ID) {
      availableTransports.push(
        http(`${getNetwork(chain.id)?.rpcUrls.infura.http[0]}/${env.NEXT_PUBLIC_INFURA_PROJECT_ID}`, {
          batch: true
        })
      )
    }
  ...
  const config = getDefaultConfig({
    appName: (env.NEXT_PUBLIC_WALLETCONNECT_PROJECT_NAME as string) || '',
    projectId: env.NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID as string,
  ...
  for (const chain of config.chains) {
    ...
    if (chain?.rpcUrls.alchemy?.http[0] && env.NEXT_PUBLIC_ALCHEMY_KEY) {
      availableRPCs.push(`${chain?.rpcUrls.alchemy.http[0]}/${env.NEXT_PUBLIC_ALCHEMY_KEY}`)
    }
    if (chain?.rpcUrls.infura?.http[0] && env.NEXT_PUBLIC_INFURA_PROJECT_ID) {
      availableRPCs.push(`${chain?.rpcUrls.infura.http[0]}/${env.NEXT_PUBLIC_INFURA_PROJECT_ID}`)
    }
