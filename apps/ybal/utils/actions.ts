...
import {LPYBAL_TOKEN_ADDRESS, STYBAL_TOKEN_ADDRESS, YBAL_TOKEN_ADDRESS, ZAP_YEARN_YBAL_ADDRESS} from '@yearn-finance/web-lib/utils/constants';
...
export async function simulateZapForMinOut(props: TSimulateZapForMinOut): Promise<{shouldMint: boolean, minOut: bigint}> {
	...
	try {
		...
		const baseContract = {...wagmiProvider, address: ZAP_YEARN_YBAL_ADDRESS, abi: ZAP_BAL_ABI};
...
export async function zapBal(props: TZapYBal): Promise<TTxResponse> {
	...
	assertAddress(ZAP_YEARN_YBAL_ADDRESS, 'ZAP_YEARN_YBAL_ADDRESS');
	...
	return await handleTx(props, {
		address: ZAP_YEARN_YBAL_ADDRESS,
		abi: ZAP_BAL_ABI,
		functionName: 'zap',
		args: [
			props.inputToken,
