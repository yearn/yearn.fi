...
export const GaugeContextApp = memo(function GaugeContextApp({children}: {children: ReactElement}): ReactElement {
	...
	const [{result: allowancesMap, status: fetchAllowancesMapStatus}, {execute: refreshAllowances}] = useAsyncHookz(
	...
	const [{result: positions, status: fetchPositionsStatus}, {execute: refreshPositions}] =
		useAsyncHookz(async (): Promise<TGaugePosition[] | undefined> => {
			if (!gauges || !isActive) {
				return;
			}
	...
	const [{result: gauges, status: fetchGaugesStatus}, {execute: refreshVotingEscrow}] =
		useAsyncHookz(async (): Promise<TGauge[] | undefined> => {
			if (!isActive) {
				return;
			}
