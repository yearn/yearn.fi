...
export function VaultsListInternalMigrationRow({ currentVault }: { currentVault: TYDaemonVault }): ReactElement {
  ...
  return (
    <Link href={`/vaults-beta/${currentVault.chainID}/${toAddress(currentVault.address)}`} className={'w-full'}>
        ...
        <div className={'yearn--table-token-section'}>
          <div className={'yearn--table-token-section-item inline'}>
            <div className={'yearn--table-token-section-item-image'}>
              <ImageWithFallback
                src={`${import.meta.env.VITE_BASE_YEARN_ASSETS_URI}/tokens/${currentVault.chainID}/${toAddress(
