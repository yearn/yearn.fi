...
import {ALL_CHAINS, ALL_VAULTS_FACTORY_CATEGORIES_KEYS} from '@vaults/constants';
...
function useVaultsFactoryQueryArguments(): TQueryArgs {
	/** 🔵 - Yearn *********************************************************************************
	...
	useEffect((): void => {
		...
		if (!categories || Object.values(categories).length === ALL_VAULTS_FACTORY_CATEGORIES_KEYS.length) {
	...
	return {
		search,
		categories: (categories || ALL_VAULTS_FACTORY_CATEGORIES_KEYS) as string[],
...
export {useVaultsFactoryQueryArguments};
