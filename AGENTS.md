...
- `src/` contains the Vite entry (`main.tsx`), a few high level shared UI elements (`components/`), hooks, utils, and TypeScript types reused across apps.
- `pages/` contains the main page elements for the site, including `v3`, `vaults` (v2), `vaults-beta` (next-gen), `portfolio`, and the landing page (`index`).
- `apps/` bundles domain flows (`landing`, `vaults` (unused), `vaults-v2`, `vaults-v3`, `nextgen`) and shared components (`lib`); keep feature-specific state, styles, and assets inside these folders.
