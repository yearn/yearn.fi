...
import {depositETH} from 'utils/actions/depositEth';
import {depositViaPartner} from 'utils/actions/depositViaPartner';
import {withdrawETH} from 'utils/actions/withdrawEth';
	...
	**********************************************************************************************/
	const allowanceFetcher = useCallback(async (inputToken: string, outputToken: string): Promise<{raw: BigNumber, normalized: number}> => {
		...
		if (!isDepositing && isOutputTokenEth) {
			spender = toAddress(process.env.ZAP_ETH_WETH_CONTRACT as string);
		}
	...
	**********************************************************************************************/
	...
	return (
		<div aria-label={'Quick Deposit'} className={'col-span-12 mb-4 mt-10 flex flex-row space-x-4 bg-neutral-200 p-8 md:mt-20'}>
			<div className={'relative z-10 w-54 space-y-2'}>
			...
			<div className={'w-14 space-y-2'}>
				...
				<Button onClick={onSwitchFromTo} className={'flex h-10 w-14 items-center justify-center bg-neutral-900 p-0'}>
