...
const resolveRpcUrl = (chainId: number): string | undefined => process.env[`VITE_RPC_URI_FOR_${chainId}`]?.trim()
...
export async function fetchPendingTimelockStrategies({
  ...
  if (!rpcUrl && !client) {
    console.warn(`Missing VITE_RPC_URI_FOR_${chainId}; pending timelock strategy lookup skipped.`)
    return []
  }
