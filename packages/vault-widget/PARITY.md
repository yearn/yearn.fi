...
- `bun run qa:vault-widget:execution-states` drives twelve deterministic
  execution states at desktop and mobile sizes without a wallet or transaction.
  Its 24 cases cover chain switching, allowance reset, approval, permit,
  execution, refresh, Safe proposal, transaction pending, Safe pending,
  cross-chain delivery, success, and failure across representative deposit,
  withdrawal, migration, rewards, yvUSD, yBOLD, and staking surfaces. It
  verifies exact step copy, widget-overlay bounds, background isolation,
  non-dismissible wallet prompts, Safe and bridge dismissal/reopen behavior,
