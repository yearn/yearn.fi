...
| Tenderly transaction-family QA | In progress | Reproducible snapshot/revert harness executes 17 transactions across ERC-4626 yvUSD, locked yvUSD cooldown/nested redemption, yBOLD zap, V3 deposit, and V2 approval migration. yvBTC is plan-only while the live vault reports `maxDeposit=0`; staking is partial-stateful because the stale VNet gauge reward callback reverts. Safe, permit, rewards, and cross-chain execution remain |
...
- `bun run qa:vault-widget:tenderly` snapshots the configured mainnet VNet,
  funds an isolated QA account, exercises package-generated plans, reports
  stateful versus environment-limited coverage, and always reverts the
  snapshot.
