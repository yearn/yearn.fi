import { usdtAbi } from '@yearn/vault-widget/internal/contracts/abi/usdt.abi'
...
export function getApproveAbi(tokenAddress?: Address) {
  return tokenAddress && TOKENS_WITH_NON_STANDARD_APPROVE_ABI.has(getAddress(tokenAddress)) ? usdtAbi : erc20Abi
}

export function requiresAllowanceResetBeforeApproval(tokenAddress?: Address): boolean {
  return Boolean(tokenAddress && TOKENS_REQUIRING_ALLOWANCE_RESET_BEFORE_APPROVAL.has(getAddress(tokenAddress)))
}
