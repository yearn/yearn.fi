...
import type { EnsoQuoteProvider, EnsoRoute, EnsoRouteRequest } from '../types'
...
export function createHttpEnsoQuoteProvider(options: HttpEnsoQuoteProviderOptions): EnsoQuoteProvider {
  ...
  return {
    async getRoute(request): Promise<EnsoRoute> {
      ...
      const route = normalizeEnsoRoute(payload, request, options.trustedRouters, options.priceImpactDivisor)
      ...
      if (
        ...
        throw new Error(`Enso route price impact exceeds the ${(options.maxPriceImpactPercent ?? 5).toFixed(2)}% limit`)
      }
