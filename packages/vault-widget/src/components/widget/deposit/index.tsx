...
export function WidgetDeposit({
  ...
  const allVaults = runtime.catalog.knownVaults
  const setIsAutoStakingEnabled = runtime.settings.setAutoStake
  ...
  const { getUsdPrice } = useVaultWidgetSpotPrices([inputToken, assetToken])
  ...
  const inputTokenPrice =
    inputToken?.address && inputToken?.chainId
      ? getUsdPrice({ address: toAddress(inputToken.address), chainId: inputToken.chainId })
      : 0
  ...
  const approvalFlowKey = useMemo(
    ...
    [account, chainId, depositAmount.debouncedBn, depositToken, destinationToken, routeType, sourceChainId]
  )
  ...
  const depositBatch = useMemo(() => {
    if ((!isWalletSafe && !supportsAtomicBatch) || !effectiveNeedsApproval) {
      return undefined
    }
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-6 pt-2 gap-3">
        ...
        <InputTokenAmount
          ...
          showTokenSelector={canOpenTokenSelector}
          inputTokenUsdPrice={inputTokenPrice}
