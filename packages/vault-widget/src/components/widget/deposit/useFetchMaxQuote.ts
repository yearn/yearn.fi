...
export const useFetchMaxQuote = ({
  ...
  const ensoRouteEndpoint = runtime.routing.ensoRouteEndpoint ?? ENSO_ROUTE_PROXY
  ...
  const fetchRoute = useCallback(
    ...
    [account, balance, chainId, depositToken, destinationToken, ensoRouteEndpoint, sourceChainId]
  )
  ...
  const normalizeVaultShares = useCallback(
      ...
      if (stakingAddress && previewRedeemCall) {
        try {
          const normalized = await readContract(config, {
            chainId: runtime.chains.resolveExecutionChainId(chainId) ?? chainId,
    ...
    [chainId, config, destinationToken, runtime.chains, stakingAddress, stakingSource, vaultAddress]
  )
