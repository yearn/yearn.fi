...
import { getTokenLogoSources } from '@yearn/vault-widget/internal/components/widget/tokenLogo.utils'
import type { useDebouncedInput } from '@yearn/vault-widget/internal/hooks/useDebouncedInput'
import type { useInput } from '@yearn/vault-widget/internal/hooks/useInput'
import { cl, formatCounterValue, formatTAmount, simpleToExact } from '@yearn/vault-widget/internal/utils'
...
export const InputTokenAmount: FC<Props> = ({
  ...
  onInputChange,
  onMaxClick,
  openLoginModal: openLoginModalOverride,
  title = 'Amount',
  ...
  const { assets, wallet } = useVaultWidgetRuntime()
  const openLoginModal = openLoginModalOverride ?? wallet.open
  const [
    {
      formValue,
      activity: [, setActive]
  ...
  const disabled = _disabled || !accountAddress
  ...
  const tokenLogoSources = useMemo(
    ...
    [assets, tokenAddress, tokenChainId, tokenLogoURI]
  )
  const zapTokenLogoSources = useMemo(
    () =>
      getTokenLogoSources({
    ...
    [assets, zapToken?.address, zapToken?.chainId]
  )
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {zapToken.address && zapToken.chainId && (
                  <TokenLogoV2
                    src={zapTokenLogoSources.src}
                    altSrc={zapTokenLogoSources.altSrc}
