...
function VaultForwardAPY({ currentVault }: { currentVault: TYDaemonVault }): ReactElement {
  ...
  const hasKelpNEngenlayer =
    currentVault.address === toAddress('0xDDa02A2FA0bb0ee45Ba9179a3fd7e65E5D3B2C90')
...
function VaultRiskScoreTag({ riskLevel }: { riskLevel: number }): ReactElement {
  ...
  return (
    <div
      className={
        'md:justify-centere col-span-2 flex flex-row items-end justify-between md:flex-col md:pt-4'
      }>
  ...
  const staked = useMemo((): TNormalizedBN => {
    ...
    if (currentVault.staking.available) {
      ...
      return toNormalizedBN(
        vaultToken.balance.raw + stakingToken.balance.raw,
  ...
  const balanceOfWrappedCoin = useYearnBalance({
    ...
    address:
      toAddress(currentVault.token.address) === WFTM_TOKEN_ADDRESS
        ? WFTM_TOKEN_ADDRESS
        : WETH_TOKEN_ADDRESS //TODO: Create a wagmi Chain upgrade to add the chain wrapper token address
  })
    ...
    <Link href={`/v3/${currentVault.chainID}/${toAddress(currentVault.address)}`} scroll={false}>
      <div
        ...
        <div
          className={cl(
            'col-span-8 z-10',
            'grid grid-cols-2 md:grid-cols-12 gap-4',
            'mt-4 md:mt-0'
          )}>
          <div
        ...
        <div
          className={
            'mt-4 flex flex-row items-center border-t border-neutral-900/20 pt-4 md:hidden'
          }>
          <VaultChainTag chainID={currentVault.chainID} />
          <button
            type={'button'}
