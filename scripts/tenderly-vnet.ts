...
export function buildTenderlyEnvFragment(params: {
  ...
  return [
    ...
    `NEXT_PUBLIC_TENDERLY_CHAIN_ID_FOR_${params.canonicalChainId}=${params.executionChainId}`,
    `NEXT_PUBLIC_TENDERLY_RPC_URI_FOR_${params.canonicalChainId}=${publicRpc}`,
    `TENDERLY_ADMIN_RPC_URI_FOR_${params.canonicalChainId}=${params.details.adminRpc || ''}`,
    `NEXT_PUBLIC_TENDERLY_EXPLORER_URI_FOR_${params.canonicalChainId}=${params.details.explorerUri || ''}`
  ].join('\n')
}
