...
import { GOVERNANCE_CHAIN_ID, YFI_ADDRESS } from '@pages/portfolio/governance/constants'
...
function getTokenLogoSrc(tokenAddress: string): string {
  return `${env.NEXT_PUBLIC_BASE_YEARN_ASSETS_URI}/tokens/${GOVERNANCE_CHAIN_ID}/${tokenAddress.toLowerCase()}/logo-32.png`
}
...
function getStatusItems(position: TGovernancePosition): string[] {
  const items: string[] = []

  if (position.walletRaw > 0n) {
    items.push(`Wallet ${formatTokenAmount(toNormalizedValue(position.walletRaw, 18))}`)
  }
  if (position.activeRaw > 0n) {
    items.push(`Active ${formatTokenAmount(toNormalizedValue(position.activeRaw, 18))}`)
  }
  if (position.cooldownRaw > 0n) {
    items.push(`Cooldown ${formatTokenAmount(toNormalizedValue(position.cooldownRaw, 18))}`)
  }
  if (position.withdrawableRaw > 0n) {
    items.push(`Withdrawable ${formatTokenAmount(toNormalizedValue(position.withdrawableRaw, 18))}`)
  }
  ...
  if (position.boostMultiplier && position.boostMultiplier > 1) {
    ...
    <div className="w-full overflow-hidden bg-surface transition-colors max-md:border-b-1 max-md:border-border">
      ...
      >
        ...
        <div className="z-10 mt-4 hidden gap-4 md:col-span-12 md:mt-0 md:grid md:grid-cols-12 md:items-center">
          ...
          <div className="yearn--table-data-section-item col-span-4" datatype="number">
            <div className="flex justify-end text-right">
              <span className="yearn--table-data-section-item-value">{tvlDisplay}</span>
            </div>
          </div>
          <div className="yearn--table-data-section-item col-span-4" datatype="number">
            <div className="flex justify-end text-right">
              <span className="yearn--table-data-section-item-value">{formatTvlDisplay(position.valueUsd)}</span>
            </div>
          </div>
        </div>
