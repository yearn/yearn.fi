    ...
    <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-4 space-y-3">
      {isBlocking ? (
        <p className="text-sm text-red-500">
          Total price impact is too high ({percentage.toFixed(2)}%). Transactions at or above{' '}
          {ZAP_SLIPPAGE_HARD_CAP.toFixed(2)}% are not supported. Try a smaller amount or use the base asset flow and
          swap elsewhere.
        </p>
      ) : (
        <p className="text-sm text-red-500">
          Total price impact is {percentage.toFixed(2)}%, above your {userTolerancePercentage.toFixed(2)}% tolerance.
