...
function makeArchiveHistoryRecord({
  ...
  strategies: TArchiveAllocationHistoryRecord['strategies']
  timestampUtc: string
  txHash?: `0x${string}`
}): TArchiveAllocationHistoryRecord {
  return {
    id,
    timestampUtc,
    blockNumber,
...
describe('buildReallocationPanels', () => {
  ...
  it('inserts archive-derived on-chain states between redis snapshots', () => {
    const olderTimestamp = '2026-04-20 08:30:00 UTC'
    const archiveTimestamp = '2026-04-21T12:00:00.000Z'
    const latestTimestamp = '2026-04-22 10:00:00 UTC'
    const currentTimestamp = '2026-04-23T14:30:00.000Z'
    const olderSnapshot = makeOptimizationRecord({
      key: 'doa:optimizations:1:1771317000',
    ...
    const archiveState = makeArchiveHistoryRecord({
      id: 'archive:manual-1',
      timestampUtc: archiveTimestamp,
      strategies: [
        {
          strategyAddress: '0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
          allocationPct: 70
    ...
    const panels = buildReallocationPanels([latestSnapshot, olderSnapshot], {}, currentAllocation, [archiveState])

    expect(panels).toHaveLength(2)
    expect(panels.map((panel) => panel.kind)).toEqual(['historical', 'historical'])
    expect(panels[0]?.annotation).toBe('On-chain update')
    expect(panels[0]?.beforeTimestampUtc).toBe(olderTimestamp)
    expect(panels[0]?.afterTimestampUtc).toBe(archiveTimestamp)
    expect(panels[0]?.afterState.strategies.map((strategy) => strategy.allocationPct)).toEqual([70, 30])
    expect(panels[1]?.annotation).toBe(null)
    expect(panels[1]?.beforeTimestampUtc).toBe(archiveTimestamp)
    expect(panels[1]?.afterTimestampUtc).toBe(currentTimestamp)
    expect(panels[1]?.afterState.strategies.map((strategy) => strategy.allocationPct)).toEqual([60, 40])
  })

  it('dedupes archive states that match redis snapshots', () => {
    const olderTimestamp = '2026-04-20 08:30:00 UTC'
    const latestTimestamp = '2026-04-22 10:00:00 UTC'
    const archiveTimestamp = '2026-04-21T12:00:00.000Z'
    const currentTimestamp = '2026-04-23T14:30:00.000Z'
