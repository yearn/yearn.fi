...
import { assert, assertAddress, MAX_UINT_256, toAddress } from '@shared/utils'
import type { TTxResponse, TWriteTransaction } from '@shared/utils/wagmi'
import { handleTx, retrieveConfig, toWagmiProvider } from '@shared/utils/wagmi'
...
export async function depositViaPartner(props: TDepositViaPartner): Promise<TTxResponse> {
  ...
  return await handleTx(props, {
    ...
    functionName: 'deposit',
    args: [props.vaultAddress, props.partnerAddress || toAddress(process.env.NEXT_PUBLIC_PARTNER_ID_ADDRESS), props.amount]
  })
}
