...
- `GET /api/holdings/ledger/protocol-return/history?address=0x...&snapshotId=snapshot_...` performs an address-only PPS protocol-return replay from the same pinned ledger.

All four Phase 3 routes are private/no-store and coexist with the legacy APIs. Development requests whose URL host is `localhost`, `127.0.0.1`, or `::1` do not require `x-admin-secret`; every non-loopback or non-development request remains admin-protected. Snapshot IDs expire 30 minutes after synchronization and verification complete. Reads verify the exact pinned head, immutable manifest, chunks, indexes, calculation version, and complete supported-chain scope. Missing, expired, corrupt, or incompatible snapshots fail closed; a route never fills missing ledger data from legacy Envio. Ledger reads bypass the revisionless daily-total and protocol-response caches. The three derived responses expose `X-Holdings-Ledger-Snapshot`, `X-Holdings-Ledger-Revision`, `X-Holdings-Ledger-Source-Generation`, and `X-Holdings-Ledger-Calculation-Version` headers so callers can assert that independently requested results used the same wallet-event revision and calculation contract. Version `canonical-envio-ledger-v2` makes the address-only PPS protocol-return semantics explicit and forces existing v1 heads through a full rebuild on their next synchronization. Last-known-good cutoffs are capped by the verified head's update time, and dated breakdowns after the pinned settled day are rejected.

Ledger protocol-return history uses only the six address-scoped wallet event streams from the pinned snapshot and does not perform live transaction-hash companion-event enrichment. It values every share receipt and exit at the vault PPS at that event timestamp, regardless of whether the user interacted directly or through a router. It still uses live Kong metadata/PPS and price providers, so revision headers identify the immutable wallet-event input and calculation contract rather than every derived-data response. The legacy protocol-return route retains transaction enrichment and exact event-asset valuation for comparison. Phase 3 deliberately performs a full replay on every ledger request; incremental protocol checkpoints remain later work.
